...
export function getLightPollutionLabel(bortle: number): string {
...
export function getLightPollutionSummary(bortle: number): string {
  return `B${bortle} · ${getLightPollutionLabel(bortle)}`;
}

// 给 UI 一个稳定的语义分段，why：颜色表达要和评分逻辑保持同一阈值
export function getLightPollutionTier(bortle: number): "dark" | "moderate" | "bright" {
